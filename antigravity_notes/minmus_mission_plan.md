...
- **Phase Modules (`DASA/phases/`)**: Individual mission stages (`boot.ks`, `ascent.ks`, `deployment.ks`, `transfer.ks`, `coast.ks`, `capture.ks`, `science_loop.ks`) are self-contained and executed sequentially by the runner.
- **Utilities (`DASA/utils/`)**: Reusable subsystems and helper functions (`logging.ks`, `telemetry.ks`, `power.ks`, `math.ks`, `coasting.ks`) are imported as needed.
